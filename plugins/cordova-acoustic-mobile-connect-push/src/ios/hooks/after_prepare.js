...
function podVariantFromPodsJson(iosDir) {
    ...
    try {
        ...
        if (libs['AcousticConnect'])
            return { name: 'AcousticConnect', spec: libs['AcousticConnect'].spec || '= 2.1.15' };
...
// environment: 'development' or 'production'. Rewrites in place whenever the
// file's current value doesn't already match — not just when the keys are
// absent — so re-running after a Debug→Release (or back) switch corrects a
// stale value instead of leaving it as whatever was written on a prior run.
function addApsEnvironmentToEntitlementsFile(filePath, environment) {
    const env = environment || 'development';
    let content = fs.readFileSync(filePath, 'utf8');
    let changed = false;

    const keys = ['aps-environment', 'com.apple.developer.aps-environment'];
    for (const key of keys) {
        ...
        if (content.includes(keyTag)) {
            const next = content.replace(valueRe, `$1${env}$2`);
            if (next !== content) {
                content = next;
                changed = true;
            }
...
// Cordova generates a separate entitlements file per build configuration
// (Entitlements-Debug.plist, Entitlements-Release.plist, ...) and
// CODE_SIGN_ENTITLEMENTS points at whichever one matches the active
// configuration — NOT the named .entitlements file, which is only a
// project-level template Xcode's UI reads and isn't itself used for
// code-signing. Apple's own convention is Debug → development APNs sandbox,
// Release/Archive → production APNs, so the Release file must carry
// aps-environment=production or a Release/Archive build silently keeps
// talking to the development APNs sandbox instead of production — this is
// exactly what previously required a manual post-hoc `-exportArchive` step
// with a hand-written exportOptions.plist to correct.
function resolveApsEnvironmentForEntitlementsFile(filename) {
    return /^Entitlements-Release\.plist$/i.test(filename) ? 'production' : 'development';
}
...
function updateEntitlements(iosDir, projectName, appGroupIdentifier) {
    ...
    for (const f of fs.readdirSync(appDir)) {
        if (f.endsWith('.entitlements') || /^Entitlements-.+\.plist$/.test(f)) {
            ...
            addApsEnvironmentToEntitlementsFile(p, resolveApsEnvironmentForEntitlementsFile(f));
        }
    }
}
